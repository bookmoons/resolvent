...
func TestQuery(t *testing.T) {
	...
	t.Run("3 responses", func(t *testing.T) {
		messages := helper.MakeMessages(t, []string{
			"eon.test A 198.51.100.2",
			"century.test A 198.51.100.3",
			"span.test A 198.51.100.4",
		})
		stub := double.NewStubQuerier(messages)
		querier := construct(t, stub, 10)
		results := []*dns.Msg{
		...
		for i := range messages {
			helper.DeepEqual(t, messages[i], results[i], "incorrect response")
		}
	})
